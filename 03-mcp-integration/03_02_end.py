...
class ECommerceAdminAgent(Workflow):
    ...
    ''' 
    Example input: add item 1 to the collection. 
    It will ask for confirmation before adding the item to the collection. If the user confirms "yes", it will add the item to the collection and return a success message. Item 1 will be added to the collection with the properties from the new_clothing_items.json file. If the user does not confirm, it will return an aborted message. It does not handle input 0 which is the first item in the list.
    '''
    @step
    async def admin(self, ev: HumanResponseEvent | AdminEvent, ctx: Context) -> InputRequiredEvent | StopEvent:
        if isinstance(ev, AdminEvent):
            ...
            return InputRequiredEvent(confirmation=f"{properties}: Please confirm that this is the item you want to add (answer yes or no)")
    ...
    '''
    Example input: "Find me some vintage floral dresses under $60"
    It will search the collection for the query and return a list of items that match the query.    
    '''
    @step
